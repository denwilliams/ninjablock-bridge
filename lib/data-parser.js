...
function parseDeviceData(data, eventEmitter) {
  ...
  var key = deviceId;
  if (deviceType === 'rfsensor') key += '_' + data.DA;

  const emit = getThrottledEmitter(key, eventEmitter);
...
function getThrottledEmitter(key, eventEmitter) {
  if (!emitters[key]) {
    emitters[key] = debounce(eventEmitter.emit.bind(eventEmitter), 1000, { leading: true, trailing: false, maxWait: 1000 });
  }
